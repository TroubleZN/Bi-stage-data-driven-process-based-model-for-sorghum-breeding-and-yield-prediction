...
def G0ET(Yield_true, envi, mg, info, father_dict, mother_dict,
    ...
    while finished == 0:
        ...
        dataset = TensorDataset(Yield_train, Geno_train, envi_train, mg_train)
        loader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
        # bar = tqdm(range(epochs))
        bar = tqdm(loader)
        bar.set_description('G0E{}T{} N_allele={}'.format(E_reduce * 1, T_reduce * 1, N_alleles))

        for (Yield_train_t, Geno_train_t, envi_train_t, mg_train_t) in bar:
            for epoch in range(epochs):
                total = 0
                # making predictions with forward pass
                Yield_pred, Treno_pred = forward(Geno_train_t, envi_train_t, mg_train_t, device)

                # calculating the loss between original and predicted data points
                # loss = loss_fn(Yield_true, Yield_pred)
                loss_train = RRMSE(Yield_train_t, Yield_pred)
        ...
        with torch.no_grad():
            ...
            print('Validation Loss = {},\tValidation Loss Old = {}'.format(loss_test.item(), loss_test_old))
        print('Best Validation Loss = {}'.format(loss_test_best))

        loss_test_wrong += 1
        if ((loss_train - loss_train_old) / loss_train_old >= 0
                or (loss_test - loss_test_best) / loss_test_best >= 0.01  # 0.05
                or ((loss_test - loss_train) / loss_train > 0.2 and loss_train < 0.3)):
            ...
            if 0 < (loss_train_old - loss_train) / loss_train_old < 0.01:
                for g in optimizer.param_groups:
                    g['lr'] = g['lr'] * 2
            ...
            with open('../Result/G0E' + str(E_reduce * 1) + 'T' + str(T_reduce * 1) + '.pkl', 'wb') as f:
                pickle.dump(res, f)

            if plot:
                fig, ax = plt.subplots()
                ax.scatter(Yield_pred.detach().cpu(), Yield_train.detach().cpu(), marker='*', color='g', alpha=0.5,
                           label='Train')
                ax.scatter(Yield_pred_test.detach().cpu(), Yield_test.detach().cpu(), marker='+', color='b', alpha=0.5,
                           label='Test')
...
def G1ET(Yield_true, envi, mg, info, father_dict, mother_dict,
    ...
    while finished == 0:
        ...
        # bar = tqdm(range(epochs))
        bar = tqdm(loader)
        bar.set_description('G1E{}T{} N_allele={}'.format(E_reduce * 1, T_reduce * 1, N_alleles))
        ...
        if ((loss_train - loss_train_old) / loss_train_old >= 0
            ...
            with open('../Result/G1E' + str(E_reduce * 1) + 'T' + str(T_reduce * 1) + '.pkl', 'wb') as f:
                pickle.dump(res, f)
        if loss_test_wrong == 10:
            finished = 1
